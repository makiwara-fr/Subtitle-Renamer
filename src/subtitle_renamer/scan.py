...
class Scanner:
    ...
    def recognize_season_episode(self, file_name):
        ...
        if tmp != None:
            split = self.regexps.get_odd_episode_splitter.split(
                self.regexps.get_odd_season_splitter.split(tmp[0])[1]
            )
            return (
                "S" + split[0].strip().lstrip("0") + "E" + split[1].strip().lstrip("0")
            )
        else:
            ...
            if tmp != None:
                ...
                if tmp != None:
                    split = self.regexps.get_odd_episode_splitter2.split(
                        self.regexps.get_odd_season_splitter2.split(tmp[0])[1]
                    )
                    return (
                        "S"
                        + split[0].strip().lstrip("0")
